...
- 2026-09-05 — Fix: Smartlead page-size caps differ per endpoint and are undocumented. `/campaigns/{id}/statistics` serves 1000; `/campaigns/{id}/leads` caps at **100**. Phase 2 raised both to 1000, and because an over-large limit answers HTTP 200 with an empty array, the leads pass silently fetched nothing — which meant no reply threads either, so a whole re-import completed "successfully" having imported no replies. Caps are now per endpoint and the empty-first-page guard covers leads as well as statistics. Never raise either without checking that endpoint first.
- 2026-09-05 — Fix: imported replies were classifying as `uncategorized` because Smartlead's `lead_category` and the prompting send time live on the *statistics* row, not in the message thread, and the importer never carried them across. The first production sweep proved it — 1,485 replies classified, 0 with a Smartlead label. The importer now keeps a per-campaign email→(category, ignore_reply, sent_at) map from the stats pass and attaches it via `SetReplyImportMeta`, which also resets `classifier_version` so the next sweep re-reads with the new evidence. Migration 14 adds `inbound_replies.prompt_sent_at`, which is what lets the 2-minute auto-reply latency rule work on historical data — text rules alone caught 139 of 1,513, while a live sample suggested over half are machine-generated.
